...
const INSTRUCTIONS = [
  ...
  "- A write that depends on a position the chain already holds — borrowing against collateral, repaying or withdrawing, spending a token that needs an allowance — is checked before it is proposed. Read the position first (the lending account data, the balance, the allowance), and if it cannot succeed, say so plainly in one sentence with the number that is missing and what would fix it, instead of proposing a card that is certain to fail.",
